# https://raw.githubusercontent.com/dustinkirkland/byobu/master/usr/share/byobu/keybindings/f-keys.tmux
###############################################################################
...
# Clear the slate
source ~/.config/tmux/f-keys.tmux.disable
...
bind-key -n M-F2 display-panes \; split-window -h -c "#{pane_current_path}"
...
# bind-key -n F5 source $BYOBU_PREFIX/share/byobu/profiles/tmujrc
...
# bind-key -n M-F11 break-pane bind-key -n C-F11 join-pane -h -s :. -t :-1
bind-key -n S-F11 resize-pane -Z
bind-key -n S-F12 source ~/.config/tmux/f-keys.tmux.disable \; display-message "Byobu F-keys: DISABLED"
# bind-key -n C-S-F12 new-window $BYOBU_PREFIX/lib/byobu/include/mondrian
bind-key -n M-F12 source ~/.config/tmux/mouse.tmux.enable
